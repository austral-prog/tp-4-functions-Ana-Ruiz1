...
def final_price(price, quantity, discount_pct, tax_pct):
    ...
    precio_con_descuento = apply_discount(subtotal, discount_pct)
    precio_con_impuesto = apply_tax(precio_con_descuento, tax_pct)
...
def best_deal(price_a, qty_a, disc_a, price_b, qty_b, disc_b, tax_pct):
    ...
    """
    precio_finala = final_price(price_a, qty_a, disc_a, tax_pct)
    precio_finalb = final_price(price_b, qty_b, disc_b, tax_pct)

    if precio_finala <= precio_finalb:
        return "A"
    else:
        return "B"
